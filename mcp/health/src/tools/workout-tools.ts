/**
 * Workout tracking tools (exercises, progress, etc.)
 */
...
export const WORKOUT_TOOLS: ToolDefinition[] = [
  {
    ...
    zodSchema: {
      text: z.string().min(1).describe("Natural language workout description (e.g., 'floor press 4x12 @45, dumbbell row 3x8 @35')"),
...

  {
    ...
    zodSchema: {
      day_of_week: z.number().int().min(1).max(7).describe("Day of week (1-7, where 1=Monday, 7=Sunday)"),
    },
    jsonSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    zodSchema: {
      limit: z.number().int().min(1).max(100).optional().describe("Number of summaries to retrieve (default: 10)"),
    },
    jsonSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    zodSchema: {
      day_of_week: z.number().int().min(1).max(7).optional().describe("Optional: filter by day of week (1-7)"),
    },
    jsonSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    zodSchema: {
      ...
      limit: z.number().int().min(1).max(200).optional().describe("Number of sessions to retrieve (default: 50)"),
    },
    jsonSchema: {
      type: 'object',
      properties: {
