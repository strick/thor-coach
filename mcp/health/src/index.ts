...
/**
 * Health MCP Server - HTTP Transport (StreamableHTTP)
 * Exposes health event logging tools via proper MCP protocol over HTTP for Docker
 */
...
const PORT = process.env.MCP_PORT ? parseInt(process.env.MCP_PORT) : 3005;
const app = express();
...
const apiClient = new HealthApiClient();
...
async function main() {
  ...
  app.listen(PORT, () => {
    console.log(`\n🏥 Health MCP Server (HTTP) running at: http://localhost:${PORT}`);
