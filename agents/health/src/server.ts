/**
 * Health Agent Server
 * Provides conversational AI interface for health event logging via /chat endpoint
 * Uses health-mcp server for all tool calls
 */

import express from 'express';
import cors from 'cors';
import 'dotenv/config';
import { HealthAgent, ChatMessage } from './agent.js';
...
agent.start()
  .then(() => {
    agentReady = true;
    console.log('✅ Health Agent ready with MCP backend');
  })
  .catch((error) => {
    console.error('❌ Failed to start health agent:', error);
    process.exit(1);
  });
...
// Start server
app.listen(PORT, () => {
  console.log(`\n🏥 Health Agent running at: http://localhost:${PORT}`);
  console.log(`📍 Chat endpoint: POST http://localhost:${PORT}/chat`);
  console.log(`💬 Send messages like: {"message": "I slept 8 hours last night"}\n`);
